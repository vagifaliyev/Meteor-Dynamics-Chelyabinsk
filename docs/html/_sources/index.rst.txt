...
Assuming no ablation, gravity and lift, and a flat earth, 
the resulting simplifyed system of ODEs can be solved exactly

.. math::   \frac{dv}{dt}  = \frac{-C_D\rho_a A v^2}{2 m} = \frac{-C_D\rho_0 A}{2 m}e^{-z/H}v^2
  :label: simple_de


.. math::   \frac{dz}{dt}  = -v\sin\theta

.. math::   \frac{dx}{dt} = v\cos\theta

By the chain rule 

.. math::   \frac{dv}{dz} = \frac{dv}{dt} \cdot \frac{dt}{dz} = \left[\frac{-C_D\rho_0 A}{2 m}e^{-z/H}v^2 \right] \cdot \frac{1}{-v\sin\theta}


Rearranging we get

.. math::   \frac{dv}{v}  = \frac{C_D\rho_0 A}{2 m\sin\theta}\,e^{-z/H} dz 

Integrating both sides to obtain the solution (writing log for the natural log throughout): 

.. math::   \log(v) + C  = \frac{-H C_D\rho_0 A}{2 m\sin\theta}e^{-z/H}

For clarity let's define the constant 

.. math::   \beta  = \frac{-H C_D\rho_0 A}{2 m\sin\theta}

Then initial condition when :math:`z = z_0, v = v_0 \implies 
C =  \beta e^{-z_0/H}-\log(v_0)`, so our solution is 

.. math::  v   = \exp\left(\beta e^{-z/H}-C\right)
